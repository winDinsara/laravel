...
<body class="bg-gradient-to-br from-gray-100 via-blue-50 to-blue-100 min-h-screen flex flex-col">
    ...
    <nav class="bg-purple-600 shadow-lg">
        <div class="container mx-auto px-4 py-4 flex justify-between items-center text-white">
            <a href="{{ route('dashboard') }}" class="text-2xl font-bold hover:text-blue-200 transition">
                My Laravel App
            </a>
            <div class="flex items-center space-x-4">
                @auth
                    <span class="text-sm">Hello, <strong>{{ Auth::user()->name }}</strong></span>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <a href="{{ route('account.edit') }}" class="bg-blue-600 text-white px-2 py-2 rounded hover:bg-blue-700 transition">Manage Account</a>
                ...
                @else
                    ...
                    <a href="{{ route('register.form') }}" class="hover:underline">Register</a>
                @endauth
            </div>
        </div>
    </nav>
